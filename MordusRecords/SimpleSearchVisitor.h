...
// À COMPLÉTER: Initialiser tous les attributs
	: m_compareOp(OP_COMPARE()), m_searchPropertyInitialized(false), m_datePropertyPtr(nullptr), m_floatPropertyPtr(nullptr), m_intPropertyPtr(nullptr),
	m_sectionPropertyPtr(nullptr), m_stringPropertyPtr(nullptr), m_URLPropertyPtr(nullptr), m_properties(PropertyContainer())
{
...
void SimpleSearchVisitor<OP_COMPARE>::visitDateProperty(DateProperty& prop)
{
	...
	visitDateProperty(const_cast<const DateProperty&>(prop));
...
{
	// À COMPLÉTER: utiliser la version  de la méthode recevant une propriété const
	visitFloatProperty(const_cast<const FloatProperty&>(prop));
}
...
{
	// À COMPLÉTER: utiliser la version  de la méthode recevant une propriété const
	visitIntProperty(const_cast<const IntProperty&>(prop));
}
...
{
	// À COMPLÉTER: utiliser la version  de la méthode recevant une propriété const
	visitSectionProperty(const_cast<const SectionProperty&>(prop));
}
...
{
	// À COMPLÉTER: utiliser la version  de la méthode recevant une propriété const
	visitStringProperty(const_cast<const StringProperty&>(prop));
}
...
{
	// À COMPLÉTER: utiliser la version  de la méthode recevant une propriété const
	visitURLProperty(const_cast<const URLProperty&>(prop));
}
...
void SimpleSearchVisitor<OP_COMPARE>::visitDateProperty(const DateProperty& prop)
{
	...
	else if (m_datePropertyPtr != nullptr)
	{
		if (m_compareOp(prop, *m_datePropertyPtr))
		{
...
void SimpleSearchVisitor<OP_COMPARE>::visitFloatProperty(const FloatProperty& prop)
{
	...
	else if (m_floatPropertyPtr != nullptr)
	{
		if (m_compareOp(prop, *m_floatPropertyPtr))
		{
			m_properties.push_back(prop.clone());
		}
	}
...
void SimpleSearchVisitor<OP_COMPARE>::visitIntProperty(const IntProperty& prop)
{
	...
	else if (m_intPropertyPtr != nullptr)
	{
		if (m_compareOp(prop, *m_intPropertyPtr))
		{
			m_properties.push_back(prop.clone());
		}
	}
...
void SimpleSearchVisitor<OP_COMPARE>::visitStringProperty(const StringProperty& prop)
{
	...
	else if (m_stringPropertyPtr != nullptr)
	{
		if (m_compareOp(prop, *m_stringPropertyPtr))
		{
			m_properties.push_back(prop.clone());
		}
	}
...
void SimpleSearchVisitor<OP_COMPARE>::visitURLProperty(const URLProperty& prop)
{
	...
	else if (m_URLPropertyPtr != nullptr)
	{
		if (m_compareOp(prop, *m_URLPropertyPtr))
		{
			m_properties.push_back(prop.clone());
		}
	}
