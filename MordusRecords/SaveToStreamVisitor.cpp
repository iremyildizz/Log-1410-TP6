...
void SaveToStreamVisitor::visitSectionProperty(SectionProperty& prop)
{
	...
	for (auto it = prop.begin(); it != prop.end(); ++it) {
		if (dynamic_cast<SectionProperty*>(&(*it))) {
			visitSectionProperty(dynamic_cast<SectionProperty&>(*it));
		}
		else {

			defaultVisitProperty(*it);
		}
	}
...
void SaveToStreamVisitor::visitSectionProperty(const SectionProperty& prop)
{
	...
	for (auto it = prop.cbegin(); it != prop.cend(); ++it) {
		auto* sectionProp = static_cast<SectionProperty*>(const_cast<BaseProperty*>(&(*it)));
		if (sectionProp != nullptr) {
			sectionProp->accept(*this);
		}
		else {
			defaultVisitProperty(*it);
		}
